 ...
 *
 * תיקונים בגרסה זו:
 * 1. תוקנה קריסה: getYoutubeSecureUrl → getYoutubeLink (פונקציה שלא הייתה מוגדרת)
 * 2. תוקנה קריסה: return null הוסר מהרמה הראשית — מסך הבית והשאלון
 *    לא עוד חסומים עד שספריית Leaflet מסיימת להיטען.
 *    המפה מציגה "טוען מפה..." בלבד אם Leaflet עדיין בטעינה.
 * 3. תוקן referrerpolicy → referrerPolicy (prop תקני ב-JSX)
 * 4. תוקנו שדות ערבית שהכילו עברית (regions.all, categories)
 * 5. תוקנו שדות רוסית שהכילו עברית (categories.nature, categories.river)
 * =================================================================================================
 */
...
function calculateDistance(
  ...
  if (!userLat || !userLon || !siteLat || !siteLon) return "0.0";
...
/**
 * יצירת כתובת Embed ל-YouTube
 * ✅ תוקן: הפונקציה הייתה מוגדרת בשם getYoutubeLink אך נקראה getYoutubeSecureUrl — תוקן.
 */
...
export default function TiyulifyApp() {
  ...
  // ✅ תוקן: isClientReady משמש רק לעיבוד ראשוני, לא לחסימת כל האפליקציה
  ...
  /**
   * אובייקט תרגומים (i18n)
   * ✅ תוקן: הסרת עברית משדות ערבית ורוסית
   */
  const labels: any = {
    he: {
    ...
    // ✅ תוקן: הסרת עברית מכל שדות הערבית
    ar: {
      search: "بحث عن مكان...",
      results: "نتائج البحث",
      surprise: "فاجئني",
      ...
      regions: {
        all: "كل البلاد",          // ✅ תוקן: היה "כל البلاد"
        north: "منطقة الشمال",
        center: "منطقة المركز",
        south: "منطقة الجنوب"
      },
      categories: {
        all: "الكل",               // ✅ תוקן: היה "الכל"
        water: "مياه وينابيع",
        nature: "منتزهات وطبيعة",  // ✅ תוקן: היה "منتزهات وطביעה"
        history: "تاريخ وتراث",    // ✅ תוקן: היה "تاريخ ותראת'"
        sleep: "إقامة وتخييم",     // ✅ תוקן: היה "מבית ותח'יים"
        food: "طعام ومطاعم",       // ✅ תוקן: היה "טעאם ומסאעם"
        bike: "مسارات الدراجات",   // ✅ תוקן: היה "מסאראת דראג'את"
        hiking: "مسارات المشي",    // ✅ תוקן: היה "מסאראת משׁי"
        promenade: "ممشى سياحي",   // ✅ תוקן: היה "ממשׁא סייאחי"
        beach: "شواطئ البحر",      // ✅ תוקן: היה "שואטئ אלבחר"
        river: "أنهار وجداول"      // ✅ תוקן: היה "אנהאר וג'דאול"
      }
    },
    // ✅ תוקן: הסרת עברית מהשדות הרוסיים
    ru: {
      search: "Поиск места...",
      results: "Результаты",
      surprise: "Удиви меня",
      ...
      categories: {
        ...
        nature: "Парки и природа",   // ✅ תוקן: היה "Пארקי и природа"
        ...
        promenade: "Променады",
        beach: "Пляжи",
        river: "Реки и ручьи"        // ✅ תוקן: היה "Реки ורוח'י"
      }
    }
  };
  ...
  useEffect(() => {
    ...
    // טעינה דינמית של Leaflet — רק בצד הלקוח
    Promise.all([
  ...
  const flyToCoords = (target: [number, number]) => {
    if (mapControl.current) {
      mapControl.current.flyTo(target, 14, { animate: true, duration: 2.0 });
    }
  };
  ...
  // ✅ תוקן: הסרנו את ה-"return null" הכללי שחסם את כל האפליקציה.
  // כעת מסך הבית והשאלון מתרנדרים מיד.
  // רק מסך המפה מציג "טוען..." אם Leaflet עדיין בטעינה.

  return (
    <div
      className="flex flex-col h-screen bg-white font-sans overflow-hidden"
      dir={activeLang === 'ar' || activeLang === 'he' ? 'rtl' : 'ltr'}
    >
      ...
      {/* -----------------------------------------------------------------------
          מסך השאלון (Quiz View)
          ----------------------------------------------------------------------- */}
      {activeView === 'quiz' && (
        <div className="flex-1 flex flex-col items-center justify-start md:justify-center p-6 bg-gray-50 overflow-y-auto pt-20">
          <h2 className="text-3xl md:text-6xl font-black text-gray-800 mb-10 md:mb-16 text-center">
            {labels[activeLang].style}
          </h2>
      ...
      {/* -----------------------------------------------------------------------
          מסך המפה (Map View)
          ✅ תוקן: Leaflet נבדק רק כאן, לא חוסם את שאר המסכים
          ----------------------------------------------------------------------- */}
      {activeView === 'map' && (
        <div className="flex flex-col h-full relative">
          ...
          <header className="bg-white/95 backdrop-blur-md border-b-2 p-3 md:p-5 flex flex-col gap-3 md:gap-5 z-[2000] shadow-xl">
            <div className="flex items-center justify-between w-full px-2">
              <div className="flex items-center gap-3 md:gap-8">
                ...
                <h2
                  className="text-2xl md:text-5xl font-black text-green-700 cursor-pointer italic tracking-tight uppercase"
                  onClick={() => setActiveView('home')}
                >
                  Tiyulify
                </h2>
          ...
          {/* ✅ תוקן: אם Leaflet עדיין בטעינה — מציגים spinner במקום דף ריק */}
          {(!isClientReady || !LeafletMapLib) ? (
            <div className="flex-1 flex items-center justify-center bg-gray-50">
              <div className="text-center">
                <div className="text-6xl mb-4 animate-spin">🗺️</div>
                <p className="text-gray-500 font-bold text-xl">{labels[activeLang].loading}</p>
              </div>
            </div>
          ) : (
            /* ✅ Leaflet מוכן — מרנדרים את המפה */
            (() => {
                ...
                <div className="flex-1 flex relative overflow-hidden">
                  ...
                  <div className="flex-1 relative">
                            ...
                            <Popup minWidth={340} maxWidth={400} className="square-modern-popup-container">
                              <div className="text-right font-sans p-1 overflow-hidden">
                                ...
                                <div className="w-full h-44 md:h-52 mb-4 shadow-xl rounded-[1.5rem] overflow-hidden bg-black relative border-2 border-white">
                                  {/* ✅ תוקן: getYoutubeSecureUrl → getYoutubeLink */}
                                  {(item.video && item.id !== "1") ? (
                                    <iframe
                                      key={`v-embed-${item.id}-${activeLang}`}
                                      width="100%"
                                      height="100%"
                                      src={getYoutubeLink(item.video)}
                                      title="Video Player"
                                      frameBorder="0"
                    ...
                    <div className="absolute bottom-6 left-6 z-[2000] flex flex-col gap-4">
                      <button
                        onClick={handleSurpriseMe}
                        className="bg-green-600 text-white w-16 h-16 md:w-28 md:h-28 rounded-full shadow-2xl flex flex-col items-center justify-center text-[10px] md:text-xs font-black border-4 border-white hover:bg-green-700 transition-all transform hover:scale-110 active:scale-90 shadow-green-400"
                      >
                      ...
                      <button
                        onClick={() => setActiveView('home')}
                        className="bg-white text-green-600 w-12 h-12 md:w-20 md:h-20 rounded-full shadow-2xl flex items-center justify-center text-3xl md:text-5xl border-2 md:border-4 border-green-600 hover:bg-green-50 transition-all transform hover:scale-110 active:scale-90 shadow-gray-400"
                      >
                        🏠
                      </button>
                    </div>
