...
export default function TiyulifyApp() {
  ...
  // חישוב נתונים מסוננים - משמר את החיפוש בכל השפות בו זמנית
  ...
  const shareViaWhatsApp = (item: any) => {
    ...
    const url = `https://www.google.com/maps/search/?api=1&query=${item.coords[0]},${item.coords[1]}`;
              ...
              <Marker key={item.id} position={item.coords}>
                <Popup>
                  <div className="text-right p-1 min-w-[200px] flex flex-col gap-2">
                    {item.image && (
                      <img src={item.image} alt="site" className="w-full h-32 object-cover rounded-lg shadow-sm" />
                    )}
                    <div>
                    ...
                    <a href={`https://www.waze.com/ul?ll=${item.coords[0]},${item.coords[1]}&navigate=yes`} target="_blank" className="bg-blue-600 text-white text-center py-2 rounded-md text-sm font-bold no-underline">
                      ניווט ב-Waze
                    </a>
                  </div>
                </Popup>
